...
rownames(mems) <- c()
colnames(mems) <- c("eid","mem1","mem2","mem3","lik1","lik2","lik3")
mems <- mems %>%
...
if(0){
    ...
    dens.thres <- 0.05
    plot(ef.sel,col=(dens > 0.1)+1,pch=20)
...
## rtsne coordinates
setwd("/n/groups/mitchison/Kenichi/projects/09 depmap/rdas/19q3/rtsne_original")
rt <- readRDS(file="rtsne_2492gns_20000_original_1.rds") %>%
    `colnames<-`(c("x","y"))
rt <- round(rt,3)
