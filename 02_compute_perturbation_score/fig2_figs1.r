...
ed1 <- readRDS(file="eff-dep-without-missing.rds")
...
setwd(plot_dir);setwd("genewise_relationship")
pdf("figS1A_CRIPSR_shRNA_relationship.pdf",width=5,height=5)
par(mar=c(5,5,3,3))
ed.lims <- c(-3,2)
...
setwd(plot_dir);setwd("genewise_relationship")
pdf("figS1B_eCDF_boxes.pdf",width=5,height=5)
par(mar=c(5,5,3,3))
...
ed.lims <- range(eff.1[gids,],dep.1[gids,],na.rm=T)
s <- svd(ed1,1,0)
...
pdf("fig2A_ccnd1.pdf",width=5,height=5)
gene.ed.plot(gid="595",sym="CCND1",ed.lims=ed.lims) # mget("CCND1",org.Hs.egSYMBOL2EG)
dev.off()

pdf("fig2A_ran.pdf",width=5,height=5)
gene.ed.plot(gid="5901",sym="RAN",ed.lims=ed.lims) # mget("RAN",org.Hs.egSYMBOL2EG)
dev.off()

pdf("fig2A_snapc4.pdf",width=5,height=5)
gene.ed.plot(gid="6621",sym="SNAPC4",ed.lims=ed.lims) # mget("GPX4",org.Hs.egSYMBOL2EG)
dev.off()

pdf("fig2A_actg2.pdf",width=5,height=5)
gene.ed.plot(gid="72",sym="ACTG2",ed.lims=ed.lims) # mget("GPX4",org.Hs.egSYMBOL2EG)
dev.off()
...
setwd(plot_dir);setwd("genewise_relationship")
pdf("fig2B_violin_score_4genes.pdf",width=4,height=3)
...
## Fig. 2F
pdf("fig2F_utp14a.pdf",width=5,height=5)
gene.ed.plot(gid="10813",sym="UTP14A",ed.lims=ed.lims)
dev.off()

## median efficacy scores: CRISPR (mid.e), shRNA (mid.d), perturbation score (mid.sc)
...
setwd(plot_dir);setwd("compare_eff_sel_score")
png("fig2C_scatter_crispr_cmpbd.png",width=500,height=500)
par(mar=c(5,5,3,3))
...
## Fig. 2D
setwd(plot_dir);setwd("compare_eff_sel_score")
write.csv(df.dns.1,file="fig2D_shrna-sensitive.csv")
...
setwd(plot_dir);setwd("compare_eff_sel_score")
png("figS1C_scatter_shrna_cmbd.png",width=500,height=500)
par(mar=c(5,5,3,3))
...
## Fig. S1D : correlation - median value
mids <- cbind(e=mid.e,d=mid.d,sc=mid.sc)
cors <- cor(mids,method="spearman")
rownames(cors) <- colnames(cors) <- c("CRISPR","shRNA","Perturbation score")

setwd(plot_dir);setwd("compare_eff_sel_score")
write.csv(cors,file="figS1D_correlation_mid_scores.csv")


if(0){
	## [NOTE] this can run this block only after '03_find_essential_genes' is run. ## 
	...
	### scatter plot
	setwd(plot_dir);setwd("compare_eff_sel_score")
	pdf("figS1E_coef_comparison.pdf",width=7,height=7)
	par(oma=c(2,2,0,0))
	par(mfrow=c(3,3))
	for(i in 1:3){
		for(j in 1:3){
	...
	## Fig. 2E : correlation of pairwise correlation between CRISPR and perturbation
	### scatter plot
	th <- .4
...
if(0){
	## Fig. 2G : clusters contianing UTP14
	## [NOTE] this can run this block only after '04_cluster_hierarchy' is run. ## 
